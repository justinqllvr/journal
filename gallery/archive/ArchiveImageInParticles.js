...
canvas.addEventListener("mousemove", (e) => {
  ...
  mouse.y = 1 - e.offsetY / canvas.height;
});
...
const createParticlesImage = (x, y, texture) => {
  ...
  //Loop through pixels
  const pixelsToSkip = 1;

  const numPoints = width * height;
  const numParticles = Math.ceil(numPoints / pixelsToSkip);
  ...
  for (let i = 0; i < numParticles; i += pixelsToSkip) {
    ...
    offsets[originalIndex * 3 + 0] = colIndex;
    offsets[originalIndex * 3 + 1] = rowIndex;
    offsets[originalIndex * 3 + 2] = 0;
